...
class blogsController {
    static async postBlogs(req, res) {
        ...
        try {
            const blogs = await blogs_1.Blogs.create({ blogTitle, blogDescription, blogImage });
            if (blogs) {
    ...
    static async getAllBlogs(req, res) {
        try {
            const blogs = await blogs_1.Blogs.find();
            if (blogs) {
    ...
    static async getOneBlogs(req, res) {
        ...
        try {
            const blogs = await blogs_1.Blogs.findById(blogId);
            if (blogs) {
    ...
    static async deleteOneBlogs(req, res) {
        const blogId = req.params.id;
        try {
            const blogs = await blogs_1.Blogs.findByIdAndDelete(blogId);
            if (blogs) {
                return (0, successMessage_1.successMessage)(res, 200, `blogs retrived`, blogs);
            }
            else {
    ...
    static async updateBlogs(req, res) {
        const blogId = req.params.id;
        try {
            const blogs = await blogs_1.Blogs.findByIdAndUpdate(blogId, req.body, { new: true });
            if (blogs) {
    ...
    static async deleteAllBlogs(req, res) {
        const blogId = req.params.id;
        try {
            const blogs = await blogs_1.Blogs.deleteMany();
            if (blogs) {
    ...
    static async likeBlog(req, res) {
        try {
            const blog = await blogs_1.Blogs.findById(req?.params?.id);
            if (!blog) {
